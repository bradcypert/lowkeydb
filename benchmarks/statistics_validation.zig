...
const StatisticsValidator = struct {
    ...
    pub fn validateStatistics(self: *StatisticsValidator, test_name: []const u8) bool {
        ...
        if (active_transactions > 0) {
            std.debug.print("    Warning: {} transactions still active\n", .{active_transactions});
        } else {
        ...
        _ = self.db.delete(test_key) catch |err| {
            std.debug.print("    Warning: Failed to clean up test key: {}\n", .{err});
        };
...
pub fn main() !void {
    ...
    // Test 3: Concurrent operations with statistics validation
    {
        ...
        const workers = try allocator.alloc(ValidationWorker, num_threads);
